...
In particular, we'll start by taking a look at the file `goodreads_interactions.csv`.
...
We will start by using pandas to get a quick overview of the goodreads dataset.
...
9999
```
As you can see, this is a small sample of our full dataset.
...
After exploring this smaller dataset, we will switch to the larger dataset later in this lab.
...
There are alternative python libraries like [dask](https://docs.dask.org/en/stable/) designed to be more efficient,
but they are awkward to use.
...
> Our [benevolent dictator for life (BFDL)](https://en.wikipedia.org/wiki/Benevolent_dictator_for_life) [Guido van Rossum](https://en.wikipedia.org/wiki/Guido_van_Rossum) began development of the python programming language in the 1980s in order to overcome limitations like this with shell scripting and make programming more user friendly.
...
In general, `CTRL-C` can be used to stop any long-running command line program.
...
> Later on, we will use the file `goodreads_reviews_dedup.json.gz` to extract user reviews.
...
You can change the output to be the more human-friendly markdown format with the commands:
```
...
> **Note:**
>
> Commands starting with a `.` like `.import` and `.mode` are specific to the `sqlite3` command and will not work with other database engines.
